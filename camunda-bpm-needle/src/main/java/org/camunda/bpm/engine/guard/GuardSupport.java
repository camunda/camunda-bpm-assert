...
public final class GuardSupport {
    ...
    /**
     * Decides whether to call {@link #checkPreconditions(DelegateExecution)} or {@link #checkPostcondions(DelegateExecution)}.
     * @param guard
    ...
    public static void dispatch(final Guard guard, final String eventName, final DelegateExecution execution) {
        ...
        if (isPrecondition.apply(eventName)) {
            ...
            guard.checkPostcondions(execution);
        }
    }
     ...
     * @return <code>true</code> if VARIABLE_SKIP_GUARDS is set, <code>false</code> else
     */
