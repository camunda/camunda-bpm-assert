...
/**
 * Adds {@link Guard}s dynamically while parsing in standalone engine. Configured in camunda-needle.cfg.xml.
 * To activate the guards, add a Properties File "camunda-guards.properties" to "src/test/resources". The file should map activity-Id to Guard implementation
 * class.
 * @author Jan Galinski, Holisticon AG
 */
public class GuardBpmnParseListener extends AbstractBpmnParseListener {

    static final String RESOURCE_BUNDLE_GUARDS = "camunda-guards";
     ...
     */
    static Function<String, Properties> LOAD_GUARD_PROPERTIES = new Function<String, Properties>() {
        ...
        public Properties apply(final String resourceBundleName) {
            ...
            try {
                ...
                LoggerFactory.getLogger(getClass()).warn("Resource bundle not found: " + resourceBundleName);
            }
    ...
    /**
     * Adds aa {@link ActivityGuard} on {@link ExecutionListener#EVENTNAME_START} and {@link ExecutionListener#EVENTNAME_END} if one is configured in the
     * properties file.
     * @param activity the current Activity in process
     */
    ...
    private String guardClass(final ActivityImpl activity) {
        return guards.getProperty(activity.getActivityId());
    }
