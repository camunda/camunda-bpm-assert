...
public class TransactionSubProcessTest extends PluggableProcessEngineTestCase {
    ...
    public void testSimpleCaseTxSuccessful() {
        ...
        final ProcessInstance processInstance = runtimeService.startProcessInstanceByKey("transactionProcess");
        ...
        final List<String> activeActivityIds = runtimeService.getActiveActivityIds(processInstance.getId());
        ...
        final EventSubscriptionEntity eventSubscriptionEntity = (EventSubscriptionEntity)createEventSubscriptionQuery().eventType("compensate")
                .activityId("tx").executionId(processInstance.getId()).singleResult();
        ...
        final Execution eventScopeExecution = runtimeService.createExecutionQuery().executionId(eventSubscriptionEntity.getConfiguration()).singleResult();
        ...
        assertEquals(1, createEventSubscriptionQuery().eventType("compensate").activityId("undoBookFlight").executionId(eventScopeExecution.getId())
                .count());
        assertEquals(1, createEventSubscriptionQuery().eventType("compensate").activityId("undoChargeCard").executionId(eventScopeExecution.getId())
                .count());
    ...
    public void testNestedCancelInner() {
        ...
        final Task taskInner = taskService.createTaskQuery().taskDefinitionKey("innerTxaskCustomer").singleResult();
        final Task taskOuter = taskService.createTaskQuery().taskDefinitionKey("bookFlight").singleResult();
    ...
    public void testMultiInstanceTx() {
        ...
        final List<EventSubscription> eventSubscriptionEntities = createEventSubscriptionQuery().eventType("compensate").list();
        ...
        final String executionId = eventSubscriptionEntities.get(0).getExecutionId();
        for (final EventSubscription EventSubscriptionEntity : eventSubscriptionEntities) {
            if (!executionId.equals(EventSubscriptionEntity.getExecutionId())) {
                fail("subscriptions not under same execution");
            }
        }

        final Task task = taskService.createTaskQuery().listPage(0, 1).get(0);
    ...
    public void testMultiInstanceTxSuccessful() {
        ...
        final List<Execution> executions = runtimeService.createExecutionQuery().activityId("receive").list();
        ...
        try {
            repositoryService
                    ...
                    .addClasspathResource(
                            "org/camunda/bpm/engine/test/bpmn/subprocess/transaction/TransactionSubProcessTest.testMultipleCancelBoundaryFails.bpmn20.xml")
                    .deploy();
    ...
    public void testCancelBoundaryNoTransactionFails() {
        try {
            repositoryService
                    .createDeployment()
                    .addClasspathResource(
                            "org/camunda/bpm/engine/test/bpmn/subprocess/transaction/TransactionSubProcessTest.testCancelBoundaryNoTransactionFails.bpmn20.xml")
                    .deploy();
            fail("exception expected");
        } catch (final Exception e) {
    ...
    public void testCancelEndNoTransactionFails() {
        try {
            repositoryService
                    .createDeployment()
                    .addClasspathResource(
                            "org/camunda/bpm/engine/test/bpmn/subprocess/transaction/TransactionSubProcessTest.testCancelEndNoTransactionFails.bpmn20.xml")
                    .deploy();
            fail("exception expected");
        } catch (final Exception e) {
